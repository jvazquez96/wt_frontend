...
export class CreateSubProblemComponent implements OnInit, AfterViewInit {
  ...
  @Input() problemFunctionCode: string; // This is the correct method
  problemTemplateCode: string; // This is the skeleton of the whole program, it contains the control character
  @Input() problemSignatureCode: string; // This is the method's skeleton that will be given to the student
  ...
  problemTypeFlag: number = 1;

  // // selectedLanguage: string, selectedDifficulty: string, selectedTopicId: number
  // selectedLanguage: string; // selected language of main/parent problem
  // selectedDifficulty: string; // selected difficulty of main/parent problem
  // selectedTopicId: number; // selected topic ID of main/parent problem
  @Input() selectedLanguage: string; // selected language of main/parent problem
  @Input() selectedDifficulty: string; // selected difficulty of main/parent problem
  @Input() selectedTopicId: number; // selected topic ID of main/parent problem
  @Input() problemNo: number; // Number representing which function/subproblem this is
  @Input() mainName: string; // Name of the parent problem
  @Output() imReadyEvent = new EventEmitter<boolean>();

  // @Input() completeSignature: string; // contains all the signature of the parent problem
  // @Input() completeFunction: string; // contains all the functions of the paretn problem
...
int main() {
    ...
    console.log(this.problemFunctionCode);
    console.log(this.problemSignatureCode);
      ...
      .subscribe(
        data => {
          ...
          if (data['status'] == "error") {
            ...
            // ----------------
            let err = "error-feedback" + this.problemNo;
  ...
  /**
   * This function returns false if the parent name has not been given
   */
  emptyMainName(): boolean {
  ...
  createProblemRequest() {
    ...
    // Get the id of the parent problem
    console.log("MAIN NAME: ", this.mainName);
    let request = {
      "name": this.mainName
    }
    this._httpProblemsService.getProblemID(request).
    subscribe(
      data => {

        this.belongs = data["id"];
        console.log("BELONGS: ", this.belongs);
        ...
        // change the source code of the subproblem content
        // this.subproblemContent.source_code = problemObject["code"];
        // this.subproblemContent.index = this.problemNo;
        ...
        this._httpProblemsService.createNewProblem(problemObject)
          .subscribe(
            data => {
              ...
              this.displayLoader = false;
              this.feedback = "Sucess: Problem successfully created!";
              this.isError = false;
              this.displayError = true;
              setTimeout(() => {
                this.displayError = false;
      ...
      error => {
        ...
        document.getElementById(err).style.display = "block";
        document.getElementById(err).style.display = "block";
          this.feedback = "Error: Couldn't get parent's ID";
          this.isError = true;
          this.displayError = true;
          setTimeout(() => {
            this.displayError = false;
