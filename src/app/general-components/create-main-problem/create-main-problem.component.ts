...
import { Subproblem } from './../create-sub-problem/Subproblem';
import {TopicsService} from "../../services/topics.service";
import {TestCasesCreatorComponent} from "../create-problem/test-cases-creator.component"
import {Router} from "@angular/router";
import {CreateSubProblemComponent} from "../create-sub-problem/create-sub-problem.component";

@Component({
  selector: 'app-create-main-problem',
  providers: [SupportedLanguages, ProblemDifficulties, EvaluatorService],
  templateUrl: './create-main-problem.component.html',
  styleUrls: ['./create-main-problem.component.css']
})
export class CreateMainProblemComponent implements OnInit, AfterViewInit {

  // Local reference variables to the 4 code editors of type code-editor
  ...
  // -- ADDED -- 
  // tells how many subproblems there will be
  // TODO: Change type any to hold a subproblem object
  subproblems: number[];
  totalParts: number;
  subproblemsReady: boolean[];
  // Tells if the parent problem has been submitted succesfully
  parentReady: boolean;
  ...
  ngOnInit() {
    ...
    this.subproblems = [0, 0];
    this.subproblemsReady = [false, false];
    this.totalParts = 2;
...
int main() {
  ...
  evaluteTestCases(selectedLanguage: string, selectedDifficulty: string, selectedTopicId: number) {
    ...
    // this._router.navigate(['/createSubProblem', document.getElementById('how_many')]);
  }
  ...
  createProblemRequest() {
    ...
    this._httpProblemsService.createNewProblem(problemObject)
      .subscribe(
        ...
        error => {
          console.log("Error al crear problema");
          this.displayLoader = false;
          document.getElementById('error-feedback').style.display = "block";
          this.feedbackCard.hideFeedbackCard("error", error);
          this.parentReady = false;
    ...
    // this.parentInfo["language"] = this.problemProgLang;
    // this.parentInfo["difficulty"] = this.problemDifficulty;
    // this.parentInfo["topic"] = String(this.problemTopicID);
    // this._shareData.changeData(this.parentInfo);
  ...
  // ---------------- ADDED ----------------
  /**
   * This functions increases amount of subproblems
   */
  increaseSubs(): void {
    if(this.totalParts + 1 <= 4) {
      this.totalParts = this.totalParts + 1;
      this.subproblems.push(0);
      this.subproblemsReady.push(false);
      // let newSub = new Subproblem(this.totalParts, null, false);
      // this.subsStates.push(newSub);
      console.log(this.subproblems);
      // console.log(this.subsStates);
    } 
  }

  /**
   * This functions decreases amount of subproblems
   */
  decreaseSubs(): void {
    if(this.totalParts - 1 >= 2) {
      this.totalParts = this.totalParts - 1;
      this.subproblems.pop();
      this.subproblemsReady.pop();
      // this.subsStates.pop();
      console.log(this.subproblems);
      // console.log(this.subsStates);
    }
  ...
  /**
   * This functions makes sure you cant create more than 4 subproblems
   */
  checkIncrease(): boolean {
    return this.totalParts == 4;
  }

  /**
   * This functions makes sure you cant create less than 2 subproblems
   */
  checkDecrease(): boolean {
    return this.totalParts == 2;
  }

  // function that receives a subproblem's state (accepted or not - true or false)
  // $event is the state - true or false (the class file is in create-subproblem folder)
  // it represent which subproblem is ready
  receiveStatus($event, it){
    // update state of subproblem at "it"
    this.subproblemsReady[it] = $event; 
    // check if all subproblems have been accepted
    // redirect to problems page if so
    let allReady = true;
    for(let i = 0; i < this.subproblemsReady.length; i++) {
      if(!this.subproblemsReady[i]) {
        allReady = false;
      }
    }
    if(allReady) {
