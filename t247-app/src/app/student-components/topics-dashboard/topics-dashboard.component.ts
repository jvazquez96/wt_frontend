...
export class TopicsDashboardComponent implements OnInit {
  ...
  // TODO: check service. There is an error on it still.
  recommendProblem() {
    let userID = JSON.parse(localStorage['userJson'])['id'];

    this.problemsService.getProblemRecommendation(userID).subscribe(
      problem => {
        console.log(problem);
      },
      error => {
        console.log("Could not get problem recommendation");
      }
    )
  }
