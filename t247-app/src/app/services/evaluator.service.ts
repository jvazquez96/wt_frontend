...
export class EvaluatorService {
  ...
  getProblemID(problem: any) {
    const headers = new Headers({'Content-Type': 'application/json', 'Access-Control-Allow-Origin': 'localhost:4200', 'Authorization': localStorage.getItem('auth_token')});
    const options = new RequestOptions({headers: headers});

    // let regexEspacio = / /;
    // let n = problem["name"];
    let n = problem["name"].replace(/ /g, "%20");

    const getProblemURL: string = environment.apiURL + '/problems/problembyname/' + n;
    return this.http.get(getProblemURL, options)
      .map((data: Response) => data.json());
  }

  getSubproblems(problem_id: string, user_id: string) {
    const headers = new Headers({'Content-Type': 'application/json', 'Access-Control-Allow-Origin': 'localhost:4200', 'Authorization': localStorage.getItem('auth_token')});
    const options = new RequestOptions({headers: headers});

    const getIDsURL: string = environment.apiURL + '/problems/listbysub-problem/' + user_id + "/" + problem_id;
    return this.http.get(getIDsURL, options)
      .map((data: Response) => data.json());
  }
